...
def play_intro_video(path=None):
    global play_video, video_thread, fade_out
    
    # Build absolute path to intro video if not provided
    if path is None:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # Try multiple intro video options
        video_options = [
            os.path.join(script_dir, "assets", "intro.mp4"),
            os.path.join(script_dir, "assets", "intro2.mp4"),
            os.path.join(script_dir, "assets", "intro3.mp4"),
        ]
    ...
    # If no video found, silently return
    if path is None or not os.path.exists(path):
        print("[LOADING] Loading intro video...at", time.ctime())
        print("[ERROR] Could not find intro video.")
        return
    
    print(f"[LOADING] Loading intro video...at {time.ctime()}")
    ...
    def _play():
        cap = cv2.VideoCapture(path)
        if not cap.isOpened():
            print("[ERROR] Could not open intro video.")
            return
        ...
        while play_video:
            ...
            # TEXT: Only show overlay in first 3 seconds
            if elapsed < 5:
            ...
            if key == 27:
                print("[SKIP] Intro skipped by user.")
                break
...
def stop_intro_video():
    ...
    if play_video:
        ...
        if video_thread and video_thread.is_alive():
            video_thread.join()
    print(f"[STOPPED] Intro stopped at {time.ctime()}")