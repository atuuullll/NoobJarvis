...
from api.stock_data import get_market_data
...
def create_market_frame(parent):
    frame = ctk.CTkFrame(parent, fg_color="#111111", border_color="#00FF99", border_width=1.5, corner_radius=10)
    frame.pack(pady=10)

    title = ctk.CTkLabel(frame, text="📈 Market Watch", font=("Consolas", 14, "bold"), text_color="#00FF99")
    title.pack(pady=(5, 2))

    text_widget = ctk.CTkTextbox(frame, height=150, width=280, fg_color="#111111", text_color="#90ee90", font=("Consolas", 12))
    text_widget.pack(padx=5, pady=5)
    text_widget.insert("end", "Loading market data...")
    ...
    def refresh():
        ...
        try:
            market_data = 1#get_market_data()
            for line in market_data:
                text_widget.insert("end", line + "\n")
        except Exception as e:
            text_widget.insert("end", "❌ Error loading data.")
        text_widget.configure(state="disabled")
        frame.after(600000, refresh)  # update every 10minute
...
def launch_gui():
    ...
    # Build proper icon path
    script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    icon_path = os.path.join(script_dir, "assets", "jarvis.ico")
    try:
        app.iconbitmap(icon_path)  # Set the icon for the main window
    except Exception as e:
        print(f"Could not set icon: {e}")
    
    root = app
    app.title("J.A.R.V.I.S.  Mark 21 - MADE BY ATUL KUMAR RAWAT")
    ...
    # Build proper avatar paths
    script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    avatar_paths = [
        os.path.join(script_dir, "avatar9.gif"),      # animated  
        os.path.join(script_dir, "face1.png"),        # static image  
        os.path.join(script_dir, "ai-avatar.jpg"),    # jpg
        os.path.join(script_dir, "ai-avatar2.jpg"),   # jpg variant
    ]
    
    # Try to load first available avatar
    for avatar_path in avatar_paths:
        if os.path.exists(avatar_path):
            show_avatar(avatar_label, avatar_path)
            break
    '''
    # Theme toggle
    def toggle_theme():
    ...
    # Create Stock Market Frame
    market_frame = create_market_frame(right_sidebar)
    market_frame.pack(side="top", anchor="n", pady=5)
    theme_widgets["frames"].append(market_frame)  # Store the market frame in theme_widgets
